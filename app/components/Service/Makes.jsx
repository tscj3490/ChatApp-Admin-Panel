...
class Makes extends React.Component {
    ...
    showModal(item, index) {
        this.setState({ viewDetail: true, selectedIndex: index, deviceId: item.parent_device, makeName: item.make_name, description: item.description })
    }
    onSubmit(e) {
        e.preventDefault()
        e.stopPropagation()
        if (this.state.selectedIndex>=0){
            let item = this.state.devices[this.state.selectedIndex]
            item.parent_device = Number(this.state.deviceId)
            item.make_name = this.state.makeName
            item.description = this.state.description
    
            Api.updateMake(item, (err, res) => {
                this.setState((old) => {
                    old.makes[this.state.selectedIndex] = res
                })
            })
        }else{
            let item = {
                parent_device: Number(this.state.deviceId),
                make_name: this.state.makeName,
                description: this.state.description    
            }
    
            Api.insertMake(item, (err, res) => {
                        ...
                        <Col lg={8}>
                            <form className="form-horizontal" onSubmit={this.onSubmit.bind(this)}>
                                ...
                                <div className="form-group">
                                    ...
                                    <div className="col-sm-9">
                                        <input id="inputContact5" type="text" value={this.state.makeName} className="form-control"
                                            onChange={(e) => this.setState({ makeName: e.target.value })} />
                                    </div>
                                </div>
                                <div className="form-group">
                            ...
                            {this.state.selectedIndex >= 0 && <div className="text-right">
                                <div style={{ cursor: 'pointer' }} onClick={() => this.deleteItem(this.state.selectedIndex)}>Delete this make?</div>
                            </div>}
                        </Col>
                    </Row>
                </div>
    ...
    deleteItem(index) {
        console.log(this.state.makes[index].ID)
        Api.getModelListWithParentID(this.state.makes[index].ID, (err, res) => {
            swal({
                title: "Are you sure?",
                text: `This make has ${res.total} models. It will be deleted!`,
                type: "warning",
                ...
                () => {
                    Api.deleteMake(this.state.makes[index].ID, (err, res) => {
                        if (err == null) {
                            this.state.makes.splice(index, 1)
                            this.setState({ makes: [...this.state.makes] })
                            swal("Deleted!", "The make has been deleted.", "success");
                        }
                    })
                });
        })
            ...
            <ContentWrapper>
                ...
                <div className="panel panel-default">
                    <div className="panel-heading">
                        <div role="button" className="pull-right text-muted" onClick={() => this.addMake()}>
