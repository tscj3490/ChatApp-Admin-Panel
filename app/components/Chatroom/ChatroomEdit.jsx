...
class ChatroomEdit extends React.Component {
        ...
        if (this.isNew) {
            ...
            if (this.state.changed) {
                ServiceApi.uploadFile(this.state.files[0], (res) => {
                    ...
                    if (res.error == null) {
                        ChatroomApi.updateChatroom(this.state.id, {
                            name: this.state.name,
                            company_name: this.state.company_name,
                            logo: this.state.logo,
                        }, (err1, res1) => {
    ...
    deleteItem(id) {
        swal({
            ...
            () => {
                ChatroomApi.deleteChatroom(id, (err, res) => {
                    if (err == null) {
    ...
    render() {
        ...
        return (
            <ContentWrapper>
                    ...
                    <Col md={8}>
                        <div className="panel panel-default">
                            <div className="panel-body">
                                <div className="h4 text-center">{this.index < 0 ? "Add New Chatroom" : "Edit Chatroom"}</div>
                                <Row className="pv-lg">
                                    <Col lg={2}></Col>
                                    <Col lg={8}>
                                        <form className="form-horizontal" onSubmit={this.submitProfile.bind(this)} >
                                            <div className="form-group">
                                                <label htmlFor="inputContact1" className="col-sm-3 control-label text-nowrap">Chatroom Name</label>
                                        ...
                                        {this.index >= 0 && <div className="text-right"><div style={{ cursor: 'pointer' }} onClick={() => this.deleteItem(this.state.id)}>Delete this Chatroom?</div>
                                        </div>}
                                    </Col>
                                </Row>
                            </div>
                ...
                {/* <Row>
                    <Col sm={6}>
                        <div className="big-button text-primary shadow-hover" onClick={() => this.viewService()}>View MyChatroom</div>
                    </Col>
                    <Col sm={6}>
